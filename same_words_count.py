...
mas1 = new_s.split(' ')  # split - метод, который разбивает сроку и делает из нее новый список
...
# raw_dic = {}  # создадим и заполним словарь, ключ - это слово, значение - число повторений слова в списке.
# for word in unique:
...
sorted_dict ={k: raw_dic[k] for k in sorted_keys }


print(f'Всего слов: {len(mas)}')
print(f'Уникальных слов: {len(unique)}')
...
# код, который давался без объяснений новичкам
